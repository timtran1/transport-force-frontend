...
export default function CronList() {
    ...
    const rows = items?.map(item => (
        <Table.Tr
            ...
            onClick={() => navigate(`/crons/${item.id}`)} className={`cursor-pointer`}
        >
            ...
            <Table.Td>{item.interval_unit}</Table.Td>
            <Table.Td><Checkbox checked={item.enabled}/></Table.Td>
        </Table.Tr>
    ))
    ...
    return (
        <main className={`max-w-screen-xl m-auto my-[50px] px-[12px] sm:px-[24px]`}>
            <div className={`flex w-full justify-between gap-2`}>
                <h1 className={`text-[36px] font-[700] text-2xl text-pr`}>{t("Scheduled Actions")}</h1>
                <Link to={`/crons/create`}>
                    <Button className={`shadow bg-primary-main text-primary-contrastText`} color={`primary`}>
                        ...
                        className={`hidden sm:inline`}>{t("Create Scheduled Action")}</span>
                    </Button>
                </Link>
            </div>
                    ...
                    <Table verticalSpacing="sm" highlightOnHover>
                        <Table.Thead>
                            <Table.Tr>
                                ...
                                <Table.Th>{t("Interval")}</Table.Th>
                                <Table.Th>{t("Interval Unit")}</Table.Th>
